#desafio 12 QEF -> Q Eu Fiz(desafio 16 NOG -> Na Ordem do Guanabara )
# from math import floor
# num = float(input('Digite um número com vírgula: '))
# Inteiro = floor(num)
# print(f'A parte inteira de {num} é {Inteiro} ')

#desafio 13 QEF -> Q Eu Fiz(desafio 17 NOG -> Na Ordem do Guanabara )
# import math
# co = float(input('Comprimento do cateto oposto: '))
# ca = float(input('Comprimento do cateto adjacente: '))
# hi = math.hypot(ca , co)
# print(f'A hipotenusa vai medir {hi:.2f}')#Dá pra fazer desse jeito aí, ou pesquisar o calculo e aplicar no codigo

#desafio 14 QEF -> Q Eu Fiz(desafio 18 NOG -> Na Ordem do Guanabara )
# from math import sin , cos , tan , radians
# angulo = float(input('Digite o ângulo que deseja: '))
# seno = sin(radians(angulo))
# cosseno = cos(radians(angulo))
# tangente = tan(radians(angulo))
# print(f'O SENO de {angulo} é {seno:.2f}')
# print(f'O COSSENO de {angulo} é {cosseno:.2f}')
# print(f'O TANGENTE de {angulo} é {tangente:.2f}')

# #desafio 15 QEF -> Q Eu Fiz(desafio 19 NOG -> Na Ordem do Guanabara )
# print('O professor quer que um dos alunos apague o quadro.')
# from random import choice
# n1 = input('Primeiro Aluno: ')
# n2 = input('Segundo Aluno: ')
# n3 = input('Terceiro Aluno: ')
# n4 = input('Quarto Aluno: ')
# lista = [n1 , n2 , n3 , n4]
# resultado = choice(lista)
# print(f'O aluno que irá ajudar o professor será o {resultado}')

#desafio 16 QEF -> Q Eu Fiz(desafio 20 NOG -> Na Ordem do Guanabara )
# from random import shuffle
# n1 = input('Primeiro Aluno: ')
# n2 = input('Segundo Aluno: ')
# n3 = input('Terceiro Aluno: ')
# n4 = input('Quarto Aluno:')
# lista = [n1 , n2 , n3 , n4]
# shuffle(lista)
# print(f'A ordem de aprensentação será:{lista}')

# #desafio 17 QEF -> Q Eu Fiz(desafio 21 NOG -> Na Ordem do Guanabara )
#Desafio de merda e professor de merda.A atividade só pode ser concluida no pycharm