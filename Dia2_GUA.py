...
*Analise de string
    ...
    #.find(O ".find" encontra objetos dentro de uma string e lhe diz em que momento começou a numeração deles.
        ...
        Boolena.Se ele não econtrar nada o false dele é indicado como um "-1")
    #.rfind(Começa a procurar o objeto pela direita)
...
*Transformação de string
    ...
    #.lower(O inverso de ".upper"(É um método vou aprender mais sobre isso futuramente) deixa
        todos os objetos de maiúsculos para minúsculos. Colocar parênteses também é essencial)
    #.capitalize((É um método vou aprender mais sobre isso futuramente)Faz com que todos os objetos
        fiquem minúsculos, e somente a primeira letra seja maiúscula. Também é essencial os parênteses)
    #.title((É um método vou aprender mais sobre isso futuramente)Faz com que cada letra inicial da palavra 
        seja maiúscula ao invés de que só a primeira letra seja maiúscula. também é essencial os parênteses)
    #.strip((É um método vou aprender mais sobre isso futuramente)Remove espaços desnecessarios no inicio 
        e no fim da str, não remove espaços no meio da string.Também é essencial os parênteses)
...
*.split(Separa as palavras em ordem e numa lista(Todos os métodos(funções com ponto)
    precisam de parênteses)
